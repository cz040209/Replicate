...
def handle_new_interaction(content, question, selected_model_id):
    # Create interaction dictionary with timestamp
    interaction = {
        "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "input_method": input_method,
    ...
    # Store user question and content preview in session history
    st.session_state.history.append(interaction)

    if content:
        # Call the API with the question and content for response
        url = f"{base_url}/chat/completions"
        data = {
            "model": selected_model_id,
            "messages": [
...
# Function to handle loading and interacting with previous interactions
def display_interaction_history():
    st.sidebar.header("Interaction History")

    for idx, interaction in enumerate(st.session_state.history):
        # Create a clickable link for each interaction in the sidebar
        if st.sidebar.button(f"View Interaction {idx + 1} - {interaction['time']}"):
            # Display the previous interaction details on the main page when clicked
            st.write(f"**Time:** {interaction['time']}")
            ...
            st.write(f"**Content Preview:** {interaction['content_preview']}")

            # Optional: Allow the user to re-ask the question or work with the content again
            re_ask_question = st.text_input("Re-ask the question", interaction["question"])
            if re_ask_question:
                st.write("Re-asking the question with the same content...")
                handle_new_interaction(interaction["content_preview"], re_ask_question, selected_model_id)

# Main logic for the app (handling input methods, displaying history, etc.)
input_method = st.selectbox("Select Input Method", ["Upload PDF", "Enter Text Manually", "Upload Audio", "Upload Image"])

# Add your logic for uploading content, handling model selection, etc.

# When a user submits a new question, store it in history
if content and question:
    handle_new_interaction(content, question, selected_model_id)

# Display interaction history with clickable options
