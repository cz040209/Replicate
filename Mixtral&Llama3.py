...
# Add a persistent chat box for asking questions
st.markdown("""
    <div class="chatbox">
        <input type="text" id="question_input" placeholder="Ask a question..." onkeyup="if(event.keyCode == 13) submitQuestion()">
    </div>
    <script>
...
# Listening for user input from the persistent chat box
if "history" not in st.session_state:
    st.session_state.history = []

# Function to process the question and generate a response from the selected model
def handle_question(question, content, selected_model_id):
    url = f"{base_url}/chat/completions"
    data = {
        "model": selected_model_id,
        "messages": [
...
# Handle the question submission via the persistent chat box
if 'question' in st.session_state:
    ...
    if question and selected_model_id:
        answer = handle_question(question, content, selected_model_id)
        
        # Add question-answer to session history
        malaysia_tz = pytz.timezone("Asia/Kuala_Lumpur")
        current_time = datetime.now(malaysia_tz).strftime("%Y-%m-%d %H:%M:%S")
        interaction = {
            "time": current_time,
        ...
        st.write(f"Answer: {answer}")
        del st.session_state['question']  # Clear the question after it's processed
